...
const exec = function (...args) {
  return new Promise((resolve) => {
    ...
    if (opts_.stdin) opts_.stdin.pipe(proc.stdin);
    if (opts_.stdout) proc.stdout.pipe(opts_.stdout);
    if (opts_.stderr) proc.stderr.pipe(opts_.stderr);
  });
};

module.exports = async function (...args) {
  const result = await exec('--reporter=spec', ...args);
  result.stdout = result.stdout.map((x) =>
    x.replace(/\b\d+ (ms|s|m|h)\b/, '_ ms')
  );
  result.stderr = result.stderr.map((x) =>
    x.replace(/\b\d+ (ms|s|m|h)\b/, '_ ms')
  );

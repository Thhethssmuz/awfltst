...
class Harness {
   ...
   */
  extend(proto) {
    if (!proto || typeof proto !== 'object') return this;
    ...
    for (const [key, desc] of Object.entries(descriptors)) {
      ...
      Object.defineProperty(ExtendedTest.prototype, key, desc);
    }
  ...
  async _run(opts) {
    for (const i of ['only', 'skip', 'group', 'skipGroup'])
      if (!Array.isArray(opts[i]) || !opts[i].length) opts[i] = null;
    ...
    const parent = {
      inspect: {
        depth: opts.depth || 3,
        colors: Boolean(opts.color),
        sorted: true,
      },
      console: Object.hasOwn(opts, 'console') ? opts.console : true,

...
const colors = require('node:util').inspect.colors;
...
class Spec {
  ...
  reportSpacing(next) {
    ...
    if (next === 'end') return;
    if (previous === 'skipped' && next === 'skipped') return;
    if (previous === 'console' && next === 'console') return;
  ...
  reportHeader(result) {
    ...
    if (!result.ok && this.opts.summary) this.errorSummary.push(line);
  }

  reportAssertion(assertion) {
    if (assertion.ok === 'skipped') return;
    ...
    if (!assertion.ok) {
      ...
      if (this.opts.summary) this.errorSummary.push(line);
    }
    this.current.depth -= 2;
  }
  ...
  reportConsole(content, streamId) {
    ...
    if (streamId === 'stderr') error(block);
    else log(block);
  }
  ...
  report(result) {
    if (result.operator !== 'chain') {
      this.test.total += 1;
      if (result.ok === 'skipped') this.test.skip += 1;
      else if (result.ok) this.test.pass += 1;
      if (this.current.depth === 0) this.duration += result.duration;
    ...
    if (result.operator !== 'chain') this.current.id.push(1);
    ...
    for (const {key, value} of result.results || []) {
      switch (key) {
        ...
        case 'chain':
          this.assert.total += 1;
          if (value.ok === 'skipped') this.assert.skip += 1;
          else if (value.ok) this.assert.pass += 1;
          this.report(value);
          break;
    ...
    this.current.depth -= 2;
    if (result.operator !== 'chain') {
      if (result.ok !== 'skipped') this.reportSpacing('end');
      this.current.id.pop();
      this.current.depth -= 2;
  ...
  summary() {
    ...
    const pluralize = (xs) => {
      for (const i of xs.keys())
        if (typeof xs[i] === 'number' && xs[i] !== 1) xs[i + 1] += 's';
      return xs;
    };
    ...
    for (let i = 0; i < table[0].length; i += 1) {
      const max = table.reduce(
        (l, row) => (row[i] ? Math.max(l, row[i].length) : l),
        0
      );
    ...
    log(table[1]);
    if (!ok) log(table[2]);
    if (this.test.skip > 0) log(table[3]);
    log(table[4]);
    log();

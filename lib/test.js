...
class Test {
  ...
  constructor(...args) {
    ...
    if (!this._opts.hasOwnProperty('console'))
      this._opts.console = true;
    if (this._opts.group && !Array.isArray(this._opts.group))
      this._opts.group = [this._opts.group];
  ...
  async run(parent, run = {}) {
    this._opts = Object.assign({group: []}, parent, this._opts);
    this._run = run;
    if (this._opts.skip ||
        run.skip && run.skip.indexOf(this._name) !== -1 ||
        run.only && (!this._opts.only && run.only.indexOf(this._name) === -1) ||
        run.group && run.group.every(x => this._opts.group.indexOf(x) === -1) ||
        run.skipGroup &&
          run.skipGroup.some(x => this._opts.group.indexOf(x) !== -1)) {
      return {
        ok      : 'skipped',
        operator: 'test',
    ...
    const byCount = (count, result) => {
      if (result.key === 'chain')
        return result.value.results.reduce(byCount, count);
      if (result.key === 'test' || result.key === 'assertion')
        return count + 1;
      return count;
    };
    const count = this._results.reduce(byCount, 0);
  ...
  compare(comparator, actual, expected, name, options = {}) {
    ...
    const result = {ok, operator,
      expected: options.hasOwnProperty('expected') ?
        options.expected :
        inspect(expected, this._opts.inspect),
      actual: options.hasOwnProperty('actual') ?
        options.actual :
        inspect(actual, this._opts.inspect),
      diffable: options.diffable || false,
      name    : name || operator,
      at      : options.at || trace()
    };

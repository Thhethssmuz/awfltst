...
const matchRunParameters = function (test, run, parent) {

  // disallow before and after hooks to be run for inline tests
  if (parent && parent.harness.tests.indexOf(test) === -1)
    return false;

  if (parent && run.once) {
    const tests = parent.harness.tests.filter(t => matchRunParameters(t, run));
    const index = tests.indexOf(test);

    if (index === -1)
      return false;

    if (run.before && index === 0)
      return true;

    if (run.after && index === tests.length - 1)
      return true;
...
class Test {
  ...
  async run(parent, run) {
    ...
    try {
      ...
      for (const instance of parent.harness.before) {
        if (matchRunParameters(this, instance._opts, parent)) {
  ...
  async throws(test, expected, name) {
    ...
    const name_ = name || (expectedIsEvaluationFunction ? 'throws' :
...
Test.prototype.hasOwnProperty =
Test.prototype.has;

Test.prototype.lackOwnProperty =
Test.prototype.lack;
